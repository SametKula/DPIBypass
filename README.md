...
## 1. Projenin Amacı
Bu proje, İnternet Servis Sağlayıcıları (İSS) tarafından uygulanan Derin Paket İnceleme (Deep Packet Inspection - DPI) ve DNS Zehirlenmesi (DNS Hijacking) yöntemlerini aşmak amacıyla geliştirilmiş yerel bir proxy sunucu uygulamasıdır. Temel hedefi, kullanıcıların standart internet erişimlerini kısıtlayan ağ engellerini atlatmaktır. Bu projede sadece macOS değil linux ve windows için de kullanılabilir şekilde kodlanmıştır.
